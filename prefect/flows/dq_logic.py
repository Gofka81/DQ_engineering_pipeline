...
# Safe globals for executing validated transform expressions (step 3 of apply_recommendations).
# __builtins__ is empty to block all built-in access. Allowed: pandas, re, and primitive casts.
...
def parse_csv(stream, has_header: bool = True) -> tuple[pd.DataFrame, int]:
    ...
    # Buffer into BytesIO because raw urllib3 responses (MinIO) are not reliably
...
def profile_dataframe(df: pd.DataFrame, malformed_rows: int = 0) -> dict[str, Any]:
    """
    Profile a DataFrame to produce objective facts. No strategy decisions are made here.
    ...
    for col in df.columns:
        ...
        # Only untyped columns can have this. Numeric, bool, and datetime columns are already validated by pandas.
...
def apply_recommendations(df: pd.DataFrame, recommendations: dict[str, Any]) -> pd.DataFrame:
    ...
    Order of operations:
        1.  Whitespace strip     — normalise string values before sentinel replacement and casting
        2.  Sentinel replacement — replace coded nulls (e.g. -999) with NaN
        3.  Transform codes      — execute validated pandas expressions (generated by LLM)
        4.  Schema cast          — coerce columns to declared types; invalid values become NaN
        5.  Fill/drop            — fill NaN (including newly coerced ones) per-column strategy
        6.  Int recast           — after fill, cast int columns that are now NaN-free to int64
        7.  Outlier treatment    — winsorise/remove/cap outliers per recommendations["outliers"]
        8.  Deduplicate          — drop duplicate rows on the cleaned data
        9.  Normalize            — min-max or z-score scale requested numeric columns
       10.  Rename columns       — apply rename_to mappings from column definitions
    """
    df = df.copy()
    ...
    # ------------------------------------------------------------------
    # Sentinel values (e.g. -999.0, 9999) are validity violations: not real measurements
    # but coded "no data" markers. Replace them before any schema cast or imputation
    # so downstream steps handle them the same as real missing values.
    ...
    # ------------------------------------------------------------------
    # 3. Execute transform codes
    # ------------------------------------------------------------------
    # transform_code is a pre-validated pandas expression added at transform time.
    # Runs before schema cast so the cast sees already-normalised values
    # (e.g. "50%" → 0.5 float, "180cm" → 180 int).
    # Damage (>5% new nulls) or any exception rolls back to the original column.
    for col, col_def in columns.items():
        ...
        if not code:
            if col_def.get("transform_hint"):
                logger.warning(
                    f"[apply 3] column '{col}' has transform_hint but no transform_code. "
    ...
    for col, col_def in columns.items():
        ...
        if col_type in ("int", "float"):
            ...
            # dt.strftime() converts NaT to the literal string "NaT". Use
    ...
    for col, col_def in columns.items():
        ...
        if strategy == "median":
            ...
            pass  # intentional: leave_null keeps nulls as-is
    ...
    # ------------------------------------------------------------------
    # 6. Int recast (only when no NaN remains in the column)
    # ------------------------------------------------------------------
    for col, col_def in columns.items():
        if col not in df.columns:
            continue
    ...
    # ------------------------------------------------------------------
    # 9. Normalize (min-max or z-score)
    # ------------------------------------------------------------------
    for col, col_def in columns.items():
        if col not in df.columns:
            continue
...
def build_recommendations(df: pd.DataFrame, profile: dict[str, Any], dq_score: float) -> dict[str, Any]:
    ...
    # MAR detection runs once at O(cols²) using scipy stats tests.
    ...
    # Strategy is selected by the _outlier_strategy() heuristic. Deterministic, no LLM override.
    ...
    # EDA snapshot for the frontend dashboard.
    # Never sent to the LLM (_lean_baseline returns only "columns").
    # Not read by apply_recommendations (only reads columns/duplicates/outliers).
    eda_cols = {}
    for col, cp in column_profiles.items():
...
def build_dropmasks(df: pd.DataFrame, profile: dict, recs: dict) -> dict:
    ...
    # One null mask per column that has a missing_values strategy
    for col, col_def in recs.get("columns", {}).items():
        ...
        # Include string sentinel values because they become NaN before drop_row executes
...
def _detect_column_type(series: pd.Series) -> str:
    ...
    # Object dtype: probe values for bool/numeric/date content
...
def _profile_numeric_column(series: pd.Series) -> dict[str, Any]:
    ...
    Outliers: IQR method, only computed when ≥30 non-null values are present.
    ...
    # Coerce to numeric to handle object columns detected as numeric (e.g. "75000")
    ...
    if len(numeric_vals) > 0:
        ...
        # Numeric coercion failed (date column). Compute min/max as ISO strings.
...
def _profile_string_column(series: pd.Series) -> dict[str, Any]:
    ...
    top_values: up to 5 most frequent values, used as context for LLM enrichment.
    """
    n_total = len(series)
    n_null = int(series.isna().sum())
    ...
    # Partial castability check: detects mixed-format columns.
...
#: Values normalised to lowercase. Comparison always strips and lowercases before checking.
...
_PATTERNS = {
    ...
    "date_iso": re.compile(r"^\d{4}-\d{2}-\d{2}$"),       # listed before phone: ISO dates also match the phone pattern
...
def _calculate_consistency(df: pd.DataFrame, total_cells: int) -> float:
    ...
    Numeric and datetime columns are skipped. Patterns do not apply to them.
    """
    if total_cells == 0:
        return 100.0
