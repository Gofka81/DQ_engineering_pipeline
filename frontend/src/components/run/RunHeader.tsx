...
export function RunHeader({ runId, filename, status, createdAt, resultExpired, sourceExpired, onDownload, onRestart, restartLoading }: Props) {
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {status === "COMPLETED" && (
          <div className="flex items-center gap-2 flex-shrink-0 self-center">
            {sourceExpired ? (
              <span
                title="Source file expired — re-run unavailable after 7 days"
                className="px-3 py-1.5 text-sm bg-gray-200 dark:bg-zinc-700 text-gray-400 dark:text-zinc-500 rounded-lg font-medium cursor-not-allowed select-none"
              >
                Adjust & re-run
              </span>
            ) : onRestart ? (
              <button
                ...
                className="px-3 py-1.5 text-sm border border-gray-300 dark:border-zinc-600 text-gray-700 dark:text-zinc-300 rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-800 font-medium transition-colors disabled:opacity-50"
              >
            ...
            {resultExpired ? (
              <span
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-sm bg-gray-200 dark:bg-zinc-700 text-gray-400 dark:text-zinc-500 rounded-lg font-medium cursor-not-allowed select-none"
              >
